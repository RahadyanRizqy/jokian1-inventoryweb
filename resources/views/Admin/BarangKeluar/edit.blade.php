            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="teknisiU" class="form-label">Teknisi</label>
                            <select name="teknisiU" id="teknisiU" required class="form-control">
                                <option value="">Pilih Teknisi</option>
                                @foreach($teknisi as $t)
                                    <option value="{{$t->teknisi_id}}">{{$t->teknisi_nama}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.length > 0) {
                    ...
                    // $("#teknisiU").val(data[0].teknisi_id);
                } else {
                    $("#loaderkdU").addClass('d-none');
                    $("#statusU").val("false");
                    $("#nmbarangU").val('');
    ...
    function submitFormU() {
        ...
        const tujuan = $("input[name='tujuanU']").val();
        const jml = $("input[name='jmlU']").val();
        const teknisi = $("select[name='teknisiU']").val();
    ...
    function resetValidU() {
        ...
        $("select[name='jmlU']").removeClass('is-invalid');
    };
